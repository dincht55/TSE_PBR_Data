...
def get_twse_bwibbu(stock_no, start_month):
...
def plot_close_and_percent_b_diff(df, stock):
    """
    繪製上下兩個子圖：
    上圖顯示 Close (股價)
...
def main(stock, start_month):
    df = get_twse_bwibbu(stock_no=stock, start_month=start_month)
    tdf = calculate_indicator(df)
    re = get_stock_close_batch(tdf.日期, stock)

    # 合拼資料
    df_dict = pd.DataFrame(list(re.items()), columns=["日期", "Close"])
    merged_df = pd.merge(tdf, df_dict, on="日期", how="left")
    plot_close_and_percent_b_diff(merged_df, stock)
